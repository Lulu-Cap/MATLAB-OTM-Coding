...
writerObj1.FrameRate = 60; % How many frames per second.
open(writerObj1); 
...
for kk = 1:Solver.time.Nt
    ...
    % Update Damping factor - Not required here
...
%     end
    ...
    if mod(kk,1000)==0
        if Solver.domain.dim == 2 % make appropriate plots for 2D scenario
            %% Displacement plot
            %subplot(1,1,1)
            plot(nd.x_start(:,1),nd.x_start(:,2),'ko','MarkerSize',1);
            hold on;
            plot(nd.x1(:,1),nd.x1(:,2),'bo');
            plot(mp.x1(:,1),mp.x1(:,2),'rx');
            axis equal;
            legend('node start','node','mp');
            hold off;
...
%             subplot(1,2,2)
%             dt = Solver.time.dt;
%             quiver(nd.x1(:,1),nd.x1(:,2),(nd.x1(:,1)-nd.x0(:,1))/dt,(nd.x1(:,2)-nd.x0(:,2))/dt);
