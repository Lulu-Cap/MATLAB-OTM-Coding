...
Solver.time.dt = 0.0001; % size of timestep [s]. Note that it is constant (not adaptive)
...
for kk = 1:Solver.time.Nt
    ...
    % Update indentor position
    nd.Dirichlet.Values = Dirichlet_Update(nd.Dirichlet.Values,Solver.time.t(kk)); 
    ...
    [mp.vol,mp.Fdot,mp.F] = update_properties(Solver,nd,mp,Shape); % Updates vol,F,Fdot
...
% Make a movie, and well as steady state plots of displacement and stress,
% since I need these for validation.