...
% Stress Functions
function stress = StressPlaneStrain(mp,Solver)
% Computes the plane strain formulation of the stress for a 2D scenario
    E = Solver.Material.E;
    nu = Solver.Material.poisson;
    C = E/((1+nu)*(1-2*nu));
    
    stress = zeros(size(mp.strain,1),4);
    
    stress(:,1) = C*( (1-nu)*mp.strain(:,1) + nu*mp.strain(:,4) ); % sigma_11
    stress(:,2) = C*( (1-2*nu) * mp.strain(:,2) ); % sigma_12
    stress(:,3) = mp.stress(:,2); % sigma_21
    stress(:,4) = C*( nu*mp.strain(:,1) + (1-nu)*mp.strain(:,4) ); % sigma_22

end

function stress = StressPlaneStress(mp,Solver)
% Computes the plane stress formulation of the stress for a 2D scenario
    ...
    stress(:,1) = C*( (1)*mp.strain(:,1) + nu*mp.strain(:,4) ); % sigma_11
    stress(:,2) = C*( (1-nu) * mp.strain(:,2) ); % sigma_12
    stress(:,3) = mp.stress(:,2); % sigma_21
    stress(:,4) = C*( nu*mp.strain(:,1) + (1)*mp.strain(:,4) ); % sigma_22
end

function stress = Stress2D(mp,Solver)
...
% I'm not even sure this has a physical meaning, but it only took a second
    ...
    stress(:,1) = E*( (1-nu)*mp.strain(:,1) + nu*mp.strain(:,4) ); % sigma_11
    stress(:,2) = E*( (1-2*nu) * mp.strain(:,2) ); % sigma_12
    stress(:,3) = mp.stress(:,2); % sigma_21
    stress(:,4) = E*( nu*mp.strain(:,1) + (1-nu)*mp.strain(:,4) ); % sigma_22
    
end

function stress = Stress3D(mp,Solver)
% Computes the stress for a full 3D simulation
    E = Solver.Material.E;
    nu = Solver.Material.poisson;
    
    stress = zeros(size(mp.strain,1),9);
    
    stress(:,1) = E*( (1-nu)*mp.strain(:,1) + nu*mp.strain(:,5) + nu*mp.strain(:,9)); % sigma_11
    stress(:,2) = E*( (1-2*nu) * mp.strain(:,2) ); % sigma_12
    stress(:,3) = E*( (1-2*nu) * mp.strain(:,3) ); % sigma_13
    stress(:,4) = mp.stress(:,2); % sigma_21
    stress(:,5) = E*( (nu)*mp.strain(:,1) + (1-nu)*mp.strain(:,5) + nu*mp.strain(:,9)); % sigma_22
    stress(:,6) = E*( (1-2*nu) * mp.strain(:,6) ); % sigma_23
    stress(:,7) = mp.stress(:,3); % sigma_31
    stress(:,8) = mp.stress(:,6); % sigma_32
    stress(:,9) = E*( (nu)*mp.strain(:,1) + nu*mp.strain(:,5) + (1-nu)*mp.strain(:,9)); % sigma_33
