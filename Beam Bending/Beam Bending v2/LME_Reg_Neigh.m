...
for mp = 1:N_mp
    ...
    while norm(lambda-lambda0) > Tol_lambda
        ...
        [H,r] = LME_Reg_Hessian(X,X_a,p,h,dim); % Hessian matrix and gradient wrt lambda
...
function [H,r] = LME_Reg_Hessian(x,xa,p,h,dim)
...
%   p: Shape function evaluations at each node 'a'
    ...
    f = -beta*dot((x-xa)',(x-xa)')' + dot(lambda',(x-xa)')'; %lambda'/h
    p = exp(f);
    p = p./sum(p);
