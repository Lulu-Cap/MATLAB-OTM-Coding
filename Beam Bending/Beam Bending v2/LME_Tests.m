...
[x,y] = meshgrid(x);
for ii = 1:length(x(:))
    x_a(ii,:) = [x(ii),y(ii)]; %nodal positions
end
...
for mp = 1:N_mp
    ...
    while norm(lambda-lambda0) > Tol_lambda
        ...
        p = P(X,X_a,lambda,beta); % Guess at shape functions
        [H,r] = LME_Reg_Hessian(X,X_a,p,dim); % Hessian matrix and gradient wrt lambda
    ...
    % Send info to output
    Shape(mp).p = p; 
    Shape(mp).gradp = LME_Reg_Grad(X,X_a,p,Hinv,dim); % May need adjustment near discontinuities
end
...
figure;
plot3(x(neigh(1:numneigh)),y(neigh(1:numneigh)),p,'bo','MarkerSize',3);
xlabel('x'),ylabel('y');zlabel('z');title("P");
figure;
plot3(x(neigh(1:numneigh)),y(neigh(1:numneigh)),gradp(:,1),'bo');
xlabel('x'),ylabel('y');zlabel('z');title("dP/dx");
figure;
plot3(x(neigh(1:numneigh)),y(neigh(1:numneigh)),gradp(:,2),'bo');
...
%   dim: dimension of problem
    ...
    for nd = 1:NumNeigh
        grad_p(nd,:) = -p(nd)*H*(x-xa(nd,:))'; %+ p(nd)*K_a*grad_beta; % This term has been temporarily ignored
    end
...
%   p: Shape function evaluations at each node 'a'
    ...
    f = -beta*dot((x-xa)',(x-xa)')' + dot(lambda',(x-xa)')';
    p = exp(f);
    p = p./sum(p);
